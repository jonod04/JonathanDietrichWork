...
nm_values = []
for item in small_mapping:
    if item[0:2] == "nm":
        nm_values.append(small_mapping[item])
nm_values
...
for item in small_mapping:
    if item[0:2] == "nm":
        if small_mapping[item].split(" ")[-1] == "Spencer":
            nm_spencer.append(item)
nm_spencer
...
# %%
dict(get_mapping("small_mapping.csv"))
...
hepburn_films = find_specific_movies(movies_copy, "Katharine Hepburn")
hepburn_films
...
movies_copy = copy.copy(movies)
wisconsin_movies = find_specific_movies(movies_copy, "Wisconsin")
wisconsin_movies
...
def bucketize_by_genre(movies):
    ...
    genre_dict = {}
    for item in movies:
        for idx in range(len(item['genres'])):
            if item['genres'][idx] not in genre_dict:
                genre_dict[item['genres'][idx]] = []
            genre_dict[item['genres'][idx]].append(item['title'])
    return genre_dict
...
after_2019 = []
for movie in movies:
    if movie['title'] in genre_dict['Music']:
        if movie['year'] > 2019:
            if 'Music' in movie['genres']:
                after_2019.append(movie['title'])
music_after_2019 = len(after_2019)
music_after_2019
...
more_9 = []
for movie in movies:
    if movie['title'] in genre_dict['Horror']:
        if movie['rating'] > 9.0:
            if 'Horror' in movie['genres']:
                more_9.append(movie['title'])
horror_movies_above_9 = more_9
...
jen_movies = []
for movie in movies:
    if 'Jennifer Aniston' in movie['cast']:
        jen_movies.append(movie)
        
jen_buck_dict = bucketize_by_genre(jen_movies)
top_length = 0
for genre in jen_buck_dict:
    genre_len = len(jen_buck_dict[genre])
    if genre_len > top_length:
        top_length = genre_len
...
for movie in movies:
    if 'Documentary' in movie['genres']:
        doc_movies.append(movie)
top_rating = 0
directors = []
for mov in doc_movies:
    rating = mov['rating']
    if rating == top_rating:
        directors = directors + mov['directors']
    elif rating > top_rating:
        top_rating = rating
        directors = mov['directors']
max_docu_rating_directors = directors
