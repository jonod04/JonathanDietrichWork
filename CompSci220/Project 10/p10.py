...
for star in stars_1_dict:
    lum = stars_1_dict[star].stellar_luminosity
    if lum != None:
        total_lum += lum
        stars_num += 1
avg_lum_stars_1 = (total_lum/stars_num)
avg_lum_stars_1
grader.check("q7")
...
stars_2_dict = get_stars('stars_2.csv')
total_age_2 = 0
stars_num_2 = 0
for star in stars_2_dict:
    ...
    if age != None:
        total_age_2 += age
        stars_num_2 += 1
avg_age_stars_2 = (total_age_2/stars_num_2)
avg_age_stars_2
grader.check("q8")
...
for star in stars_dict:
    ...
    if star[:6] == "Kepler":
        if age != None:
            count_age += 1
            age_total += age
avg_age_kepler = age_total/count_age
avg_age_kepler
...
def get_planets(planet_file, mapping_file):
    plan_data = process_csv(os.path.join("data", planet_file))
    plan_rows = plan_data[1:]
    plan_header = plan_data[0]
    try:
        json_map = read_json(os.path.join("data", mapping_file))
...
plan_2 = get_planets("planets_2.csv", "mapping_2.json")
for planet in plan_2:
...
toi_2022_c_star = stars_dict[host]
toi_2022_c_star
grader.check("q18")
...
count = 0
for planet in planets_list:
    plan_host = planet.host_name
    star_rad = (stars_dict[plan_host]).stellar_radius
    plan_rad = planet.planet_radius
    if star_rad != None and plan_rad != None and star_rad > 10:
        count += 1
        radius_total += plan_rad
avg_planet_radius_big_stars = radius_total/count
...
for star in stars_dict:
    age = stars_dict[star].stellar_age
    if age != None:
        if low_age == None or age < low_age:
            young_star = star
            low_age = age
for planet in planets_list:
    if planet.host_name == young_star:
