...
def bar_plot(df, x, y):
    df = df.set_index(x)
    df.plot.bar(xlabel = x, ylabel = y, color="black", legend = False)
...
us_top_10_rep = pd.read_sql("SELECT institution_name, (academic_reputation + employer_reputation) AS reputation FROM rankings WHERE country = 'United States' AND year = 2019 ORDER BY reputation DESC, institution_name ASC LIMIT 10", conn)
us_top_10_rep
...
top_10_countries = pd.read_sql("SELECT country, COUNT(institution_name) AS num_of_institutions FROM rankings WHERE year = 2020 GROUP BY country ORDER BY num_of_institutions DESC, country ASC LIMIT 10", conn)
top_10_countries
...
all_countries = pd.read_sql("SELECT country, COUNT(institution_name) AS num_of_institutions FROM rankings WHERE year = 2020 GROUP BY country ORDER BY num_of_institutions DESC, country ASC", conn)
new_sum = sum(all_countries.iloc[10:]['num_of_institutions'])
...
new_dict = {'country': 'Other', 'num_of_institutions': new_sum}
# TODO: properly append this new dictionary to 'num_institutions' and update 'num_institutions'
num_institutions = new_frame.append(new_dict, ignore_index=True)
...
# then create the scatter plot using the DataFrame
reputations_usa = pd.read_sql("SELECT academic_reputation, employer_reputation FROM rankings WHERE year = 2019 AND country = 'United States'", conn)
...
# then create the scatter plot using the DataFrame
top_ranked_inter_faculty = pd.read_sql("SELECT international_students, faculty_student_score FROM rankings WHERE year = 2020 GROUP BY country HAVING MIN(rank)", conn)
...
corr_df = pd.read_sql("SELECT international_students, overall_score FROM rankings WHERE country = 'United Kingdom' AND year = 2020 AND rank <= 100", conn)
correlation_df = corr_df.corr()
uk_inter_score_corr = correlation_df.iloc[0][1]
uk_inter_score_corr
...
cit_df = pd.read_sql("SELECT ((citations_per_faculty * international_faculty)/100) AS citations_per_international, overall_score FROM rankings WHERE year = 2019", conn)
cit_corr_df = cit_df.corr()
cit_per_inter_score_corr = cit_corr_df.iloc[0][1]
...
top_cit_per_inter = pd.read_sql("SELECT country, SUM((citations_per_faculty * international_faculty)/100) AS sum_inter_citations FROM rankings WHERE year = 2019 GROUP BY country ORDER BY sum_inter_citations DESC LIMIT 15", conn)
top_cit_per_inter
...
avg_cit_per_inter = pd.read_sql("SELECT country, AVG((citations_per_faculty * international_faculty)/100) AS avg_inter_citations FROM rankings WHERE rank <= 300 AND year = 2019 GROUP BY country ORDER BY avg_inter_citations DESC", conn)
avg_cit_per_inter
...
max_cit_per_inter = pd.read_sql("SELECT country, institution_name, ((citations_per_faculty * international_faculty)/100) AS max_inter_citations FROM rankings WHERE year = 2020 GROUP BY country HAVING MAX(max_inter_citations) IS NOT NULL ORDER BY max_inter_citations DESC", conn)
max_cit_per_inter
...
country_citations_inter = pd.read_sql("SELECT country, AVG(citations_per_faculty) AS avg_citations, AVG(international_faculty) AS avg_inter_faculty FROM rankings WHERE year = 2018 AND rank <= 50 GROUP BY country ORDER BY (avg_citations - avg_inter_faculty) ASC", conn)
