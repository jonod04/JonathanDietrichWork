...
if __name__ == "__main__":
    ...
    for i in range(EPISODES):
        ...
        if random.random() < epsilon:
            ...
            qs = [ Q_table[(obs,a)] for a in range(env.action_space.n) ]
        ...
        while not done:
            ...
            if random.random() < epsilon:
                ...
                qs2 = [ Q_table[(next_obs,a)] for a in range(env.action_space.n) ]
                next_action = int(np.argmax(qs2))

            if not done:
                target = reward + DISCOUNT_FACTOR * Q_table[(next_obs,next_action)]
            else:
                target = reward
